...
def groupwise_correlation_norm(fea1, fea2, num_groups):
    ...
    cost = ((fea1/(torch.norm(fea1, 2, 2, True)+1e-05)) * (fea2/(torch.norm(fea2, 2, 2, True)+1e-05))).mean(dim=2)#计算特征相关性
    assert cost.shape == (B, num_groups, H, W)
    return cost
...
def build_gwc_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            volume[:, :, i, :, i:] = groupwise_correlation(refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i], #分成40组
                                                           num_groups)
        else:
            volume[:, :, i, :, :] = groupwise_correlation(refimg_fea, targetimg_fea, num_groups)#分成40组
    volume = volume.contiguous()
    return volume
...
def build_gwc_volume_norm(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            ...
            volume[:, :, i, :, :] = groupwise_correlation_norm(refimg_fea, targetimg_fea, num_groups) #计算相关性
    volume = volume.contiguous()
    return volume
