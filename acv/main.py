...
os.makedirs(args.logdir, exist_ok=True) # 创建目录

# create summary logger 创建摘要
...
elif args.loadckpt: # 利用预训练在训练其他数据集
...
def train_sample(sample, compute_metrics=False):
    ...
    if args.attention_weights_only: # 首次训练损失
        loss = model_loss_train_attn_only(disp_ests, disp_gt, mask)
    elif args.freeze_attention_weights: # 第二次训练损失
